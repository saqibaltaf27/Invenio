...
function Products() {
	...
	const getProducts = async (sv, sc, so, scv) => {
		let result = await fetch('http://localhost:5000/api/get_products', {
			method: 'POST',
			headers: {
	...
	const deleteProduct = async (id) => {
		let result = await fetch('http://localhost:5000/api/delete_product', {
			method: 'POST',
			headers: {
				'Content-type': 'application/json; charset=UTF-8'
			},
	...
	const deleteImage = async (id) => {
		let result = await fetch('http://localhost:5000/api/delete_product_image', {
			method: 'POST',
			headers: {
				'Content-type': 'application/json; charset=UTF-8'
			},
	...
	const updateProduct = async () => {
		...
		let response = await fetch('http://localhost:5000/api/update_product', {
			method: 'POST',
			body: f,
			credentials: 'include'
		})
